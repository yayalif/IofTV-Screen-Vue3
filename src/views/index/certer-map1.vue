...
import { currentGET, currentGETPath, GETNOBASE } from "@/api";
...
const getData = async (regionCode: string) => {
  ...
  dataSetHandle(regionCode, mockData);
};
const getGeojson = (regionCode: string) => {
  ...
  // });
  return new Promise<boolean>(async (resolve) => {
    currentGETPath("geoJson",  `${regionCode}_full`).then((mapjson) => {
      console.log("API data", mapjson);
      if (mapjson) {
        code.value = regionCode;
        registerMap(regionCode, {
          geoJSON: mapjson as any,
...
const mapClick = async (params: any) => {
  // console.log('click params: ' + JSON.stringify(params));
  const regionCode = params.data.adcode;
  const geojson: any = await getGeojson(regionCode);
  const geoList = geojson.features.map((oCity:any) => {
      return {
          adcode: oCity.properties.adcode,
          name: oCity.properties.name,
          level: oCity.properties.level,
      }
    
  })
  dataSetHandle(regionCode, geoList);
